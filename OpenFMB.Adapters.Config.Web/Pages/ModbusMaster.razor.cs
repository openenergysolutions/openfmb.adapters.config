...
{
    public partial class ModbusMaster : ComponentBase
    {
            ...
            if (!string.IsNullOrEmpty(id))
            {
                ...
                await Task.Run(() =>
                {
                    ...
                    {                        
                        ...
                        var session = new Core.Models.Plugins.Session("modbus-master");

                        config.Plugins.Dnp3MasterPlugin.Sessions.Add(session);

                        var sessionConfig = session.SessionConfiguration;
                                                
                        Profile = sessionConfig.Profiles.FirstOrDefault();
                        if (Profile == null)
                        {
                            Profile = Profile.Create("SolarStatusProfile", "modbus-master");
